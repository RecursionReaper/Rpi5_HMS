...
HOME_DIR = os.path.expanduser("~")  # Get home directory path

# Configuration
EMAIL_COOLDOWN_SECONDS = 120  # 2-minute cooldown between emails
in_cooldown = False  # Flag to track cooldown state
...
def send_email(detection_frame):
    """Send an email notification when a human is detected with the frame attached."""
    global last_email_time, in_cooldown
    current_time = time.time()
    
    # Check if we're in the cooldown period
    if current_time - last_email_time < EMAIL_COOLDOWN_SECONDS:
        if not in_cooldown:
            # Only print once when entering cooldown
            cooldown_remaining = EMAIL_COOLDOWN_SECONDS - (current_time - last_email_time)
            print(f"Email cooldown active. {cooldown_remaining:.1f} seconds remaining.")
            in_cooldown = True
        return
    ...
    image_path = os.path.join(HOME_DIR, "human.jpg")
    cv2.imwrite(image_path, detection_frame)
    print(f"Detection image saved to {image_path}")
    ...
    try:
        ...
        print(f"Email sent successfully! Next email can be sent after {EMAIL_COOLDOWN_SECONDS} seconds.")
...
def process_frames():
    ...
    global running, model, in_cooldown, last_email_time
    ...
    while running:
        try:
            ...
            # If person detected, send email with the current frame
            if person_detected:
                # Make a copy to avoid race conditions when the frame is being modified
                detection_frame = annotated_frame.copy()
                
                # Add cooldown indicator to the frame if in cooldown period
                current_time = time.time()
                if current_time - last_email_time < EMAIL_COOLDOWN_SECONDS:
                    cooldown_remaining = EMAIL_COOLDOWN_SECONDS - (current_time - last_email_time)
                    cv2.putText(
                        annotated_frame, f'Email Cooldown: {cooldown_remaining:.1f}s',
                        (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2
                    )
                
                threading.Thread(target=send_email, args=(detection_frame,), daemon=True).start()
